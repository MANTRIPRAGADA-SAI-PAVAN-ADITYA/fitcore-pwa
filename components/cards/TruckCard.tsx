...
import { TRUCK_LABELS } from '../../constants/pricing';
...
const TRUCK_EMOJI: Record<string, string> = {
  mini: '🛺', lcv: '🚐', hcv: '🚛', trailer: '🚚', container: '📦',
};
...
export default function TruckCard({ truck, onPress, estimatedPrice, demandLevel, distanceKm, isUnlocked }: Props) {
      ...
      <View style={styles.body}>
        ...
        <View style={styles.topRow}>
          <View style={styles.titleWrap}>
            <Text style={styles.icon}>{TRUCK_EMOJI[truck.truck_type] ?? '🚛'}</Text>
            <View>
              <Text style={styles.typeName}>{TRUCK_LABELS[truck.truck_type].toUpperCase()}</Text>
              <Text style={styles.meta}>{truck.capacity_tons}T capacity
          ...
          {estimatedPrice ? (
            <View style={styles.priceWrap}>
              <Text style={styles.price}>₹{estimatedPrice.toLocaleString('en-IN')}</Text>
        ...
        <View style={styles.route}>
          <View style={styles.routePoint}>
            ...
            <Text style={styles.city} numberOfLines={1}>{truck.origin}</Text>
          </View>
          <View style={styles.routeStem} />
          <View style={styles.routePoint}>
            <View style={styles.dotDest} />
            <Text style={styles.city} numberOfLines={1}>{truck.destination}</Text>
          </View>
        </View>
        ...
        <View style={styles.tags}>
          <Tag
            label={new Date(truck.available_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
            color={COLORS.textSecondary} bg={COLORS.background} border={COLORS.border}
          />
          {truck.driver?.kyc_status === 'approved' && (
            <Tag label="✓ Verified" color={COLORS.success} bg={COLORS.successBg} border="#BBF7D0" />
          )}
          {truck.listing_mode === 'managed' ? (
            <Tag label="🛡 Managed" color={COLORS.textSecondary} bg={COLORS.background} border={COLORS.border} />
          ) : isUnlocked ? (
            <Tag label={`📞 ${truck.driver?.phone ?? 'Unlocked'}`} color={COLORS.info} bg={COLORS.infoBg} border="#BFDBFE" />
          ) : (
        ...
        {truck.notes ? (
          <Text style={styles.notes} numberOfLines={1}>{truck.notes}</Text>
        ) : null}
      </View>
    </TouchableOpacity>
  );
